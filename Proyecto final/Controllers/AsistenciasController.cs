...
namespace Proyecto_final.Controllers
{
    public class AsistenciasController : Controller
    {
        ...
        private void Enviar(string emisor, string password, string receptor)
        {
            ...
            string redireccionUrl = "https://localhost:44354/Asistencias/VerificarCodigo";
    ...
    <body>
        <div class='container'>
            ...
            {
                client.Send(msg);
                TempData["CodigoVerificacion"] = numero; // Guardar el código en TempData para la verificación
            }
            catch (Exception ex)
            {
        ...
        public ActionResult VerificarCodigo(string codigoIngresado)
        {
            ...
            {
                ViewBag.Error = "El campo de código ingresado es obligatorio.";
                return View();
            }

            int codigoVerificacion = (int)TempData["CodigoVerificacion"];
            TempData.Keep("CodigoVerificacion"); // Mantener el valor en TempData

            if (codigoIngresado == codigoVerificacion.ToString())
            {
                TempData.Remove("CodigoVerificacion"); // Remover el valor de TempData después de la verificación

                // Guardar la asistencia verificada en la base de datos
                Asistencias asistencia = new Asistencias
                {
                    aprendiz_id = 4, // Asigna el ID del aprendiz correspondiente
                    fecha = DateTime.Now, // Usa la fecha actua
                    tipo = "Asistencia", // Define el tipo de asistencia (puede ser entrada, salida, etc.)
                };

                db.Asistencias.Add(asistencia);
                db.SaveChanges();

                ViewBag.Mensaje = "El código de verificación ha sido validado exitosamente.";
            }
            else
            {
